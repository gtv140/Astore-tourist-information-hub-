...
.logo{text-align:center;font-size:36px;font-weight:700;color:#00eaff;text-shadow:0 0 15px #00eaff,0 0 30px #00ffff;margin-bottom:20px;font-family:'Orbitron',sans-serif;animation:glowPulse 2s infinite alternate;}
@keyframes glowPulse{0%{text-shadow:0 0 15px #00eaff,0 0 30px #00ffff;}100%{text-shadow:0 0 30px #00eaff,0 0 60px #00ffff;}}
.hero-text{text-align:center;margin-bottom:20px;font-size:16px;font-weight:500;background:linear-gradient(90deg,#00eaff,#00ff99,#ff00ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;opacity:1;transform:translateY(0);}
label{margin-top:12px;display:block;font-weight:500;font-size:14px;}
input,select,button{width:100%;padding:10px;margin-top:6px;border-radius:10px;border:none;background:rgba(255,255,255,0.1);color:#fff;font-size:14px;transition:0.3s;}
input:focus,select:focus{background:rgba(255,255,255,0.25);outline:none;}
button{background:#00eaff;color:#000;font-size:15px;font-weight:600;cursor:pointer;transition:0.3s,box-shadow 0.5s;animation:btnPulse 2s infinite alternate;}
...
.packages{display:grid;grid-template-columns:1fr;gap:15px;margin-top:15px;}
@media(min-width:600px){.packages{grid-template-columns:repeat(auto-fit,minmax(250px,1fr));}}
.package{background:rgba(0,0,0,0.4);padding:15px;border-radius:15px;box-shadow:0 0 25px #00eaff;text-align:center;transition:0.3s, transform 0.5s;}
...
.package i{font-size:30px;color:#00eaff;margin-bottom:10px;display:block;}
...
#dashboard{max-width:700px;margin:15px auto;padding:20px;background:rgba(0,0,0,0.4);border-radius:15px;backdrop-filter:blur(10px);box-shadow:0 0 25px #00eaff;}
.dashboard-section{display:none;margin-top:15px;}
.icon-menu{display:flex;justify-content:space-around;position:fixed;bottom:10px;left:0;width:100%;background:rgba(0,0,0,0.4);padding:10px 0;box-shadow:0 0 15px #00eaff;border-top:1px solid #00eaff;z-index:1000;border-radius:15px 15px 0 0;}
...
#logoutBtn{position:fixed;top:15px;right:15px;width:45px;height:45px;font-size:20px;padding:0;background:#ff4444;border:none;border-radius:50%;box-shadow:0 0 15px #ff6666;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:1000;transition:0.3s;}
#logoutBtn:hover{transform:scale(1.1);}
.whatsapp-btn{position:fixed;bottom:60px;right:15px;background:#25d366;padding:14px 16px;border-radius:50%;color:#fff;font-size:24px;text-decoration:none;box-shadow:0 0 15px #25d366;z-index:1100;transition:0.3s, box-shadow 0.5s;}
...
<div class="hero-text">Explore Astore with premium guided tours, luxury rooms, car rentals, and adventure packages. Enjoy breathtaking landscapes, cultural experiences, and hassle-free booking.</div>
...
<iframe src="https://www.google.com/maps?q=Astore,+Pakistan&output=embed" width="100%" height="250" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
</div>
...
<p>Facebook: <a href="https://www.facebook.com/share/1BoG9YCsZN/?mibextid=wwXIfr" style="color:#0ff;">Visit Page</a></p>
</div>
...
<script>
function logout(){alert("Thank you for visiting Astore Tourist Hub!");}
